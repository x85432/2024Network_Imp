...
def parse_response_2(stream_id, raw_bytes):
    ...
    for obj in lines:
        data = obj.decode().split(': ')

        if len(data) != 2: continue # skip
...
def parse_response(client_socket, stream):
    ...
    ver_and_status   = lines[0].decode()
    theData          = ver_and_status.split(' ')
    response.version = theData[0]
    response.status  = f"{theData[1]} {theData[2]}"
...
def parse_request(client_socket):
    ...
    theData = lines[0].split(b' ')
    request.method  = theData[0].decode()
    resource        = theData[1].decode()
    request.version = theData[2].decode()
