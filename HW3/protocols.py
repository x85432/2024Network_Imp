        ...
        for h in hosts:
            if h["status"] == 1:
                if len(h["history"]) == 0 or (h["history"][-1] != "<" and h["history"][-1] != "-"):
                    ...
                    history[h["id"]] = "-"
                h["remain_length"] -= 1
                if h["remain_length"] <= 0: # remain_length == 0 有兩種情況   1.送完整個封包 2.撞到別人
                    if h["collision"]: # 如果撞到了，就必須重設wait_time並且把當前的id設為 | (停止)
                        ### random time
                        ...
                        h["collision_num"] += 1
                        history[h["id"]] = "|"
                    else: # 沒撞到就代表成功了(因為remain_length <= 0)
                        h["status"] = 0
                        h["success_num"] += 1
                        h["packet_num"] -= 1
                        history[h["id"]] = ">"
...
def slotted_aloha(setting, show_history=False):
    ...
    for t in range(setting.total_time):
        ...
        for h in hosts:
            ...
            # if resending and the begining of slot
            elif h["status"] == 2 and t % setting.packet_time == 0: # TODO:  change "xxx" to the correct protocol name
                # 重送的機率為 p_resend
                r = random.random()
                if r < setting.p_resend: # Pr(true) = p_resend/range_of_r = p_resend
                    h["action_to_do"] = 1
                    h["remain_length"] = setting.packet_time
        ...
        for h in hosts:
            if h["status"] == 1:
                ...
                if h["remain_length"] <= 0:
                    if h["collision"]:
                        h["status"] = 2 # resend: slotted aloha 撞到就是重送
...
def csma(setting, one_persistent=False, show_history=False):
    ...
    for t in range(setting.total_time):
        ...
        for h in hosts:
            ...
            ### csma 不會去偵測 collision during sending，csma只會在送之前聽
    ...
    for t in range(setting.total_time):
        ...
        for h in hosts:
            ...
            ### csma 不會去偵測 collision during sending，csma只會在送之前聽
                ...
                for others in hosts: # Collision Detection
                    if others["id"] == h["id"]:
                        continue
                    if (
                        setting.link_delay >= 0
                ...
                if others_sending: # stop and abort
                    h["action_to_do"] = 3
