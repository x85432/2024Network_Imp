...
class Connection:
    ...
    def send_request(self, request):
        ...
        if 'body' in request:
            ...
            self.__send_headers(stream_id, headers, end_stream=True) # No body is the end
    ...
    def __send_headers(self, stream_id, headers, end_stream=False):
        ...
        headers_str = ""
        for hd, value in headers.items():
            headers_str += f"{hd}: {value}\r\n"
    ...
    def __send_body(self, stream_id, body):
        ...
        while len(body) > chunk_size:
            ...
            frame = http_2_frame.create_headers_frame(stream_id, payload.encode(), flags=0)
            self.send_streams[stream_id].append(frame)

        # Though len(body) is 0 here, it still send a data frame but body part is empty with flag = 1.
        # Therefore, the server still know the end of stream.
