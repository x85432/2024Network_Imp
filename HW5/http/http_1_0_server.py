...
class ClientHandler():
    ...
    <body>
        ...
        <a href="/static/{file_names[2]}">{file_names[2]}</a>
            ...
            response['headers']['Content-Length'] = len(response['body']) # Content Length
    ...
    def __do_post(self, request):
        ...
        response = self.__not_found_response() # default is 404
    ...
    def __send_file(self, request, file_path):
        ...
        response_str = f"HTTP/1.0 {response['status']}\r\n"
        for hd, value in response['headers'].items():
            response_str += f"{hd}: {value}\r\n"
        response_str += "\r\n"
    ...
    def __send_response(self, request, response):
        ...
        try:
            self.client_socket.sendall(response_str.encode()+response['body']) # send response  # body was encoded
        except:
