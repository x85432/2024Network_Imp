...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        self.add_flow(datapath=datapath, priority=0,  match=match, inst=inst, table_id=DEFAULT_TABLE)
        ...
        self.add_flow(datapath=datapath, priority=1,  match=match, inst=inst, table_id=FILTER_TABLE1)

        match = parser.OFPMatch()
        inst = [parser.OFPInstructionGotoTable(FORWARD_TABLE)]
        self.add_flow(datapath=datapath, priority=0,  match=match, inst=inst, table_id=FILTER_TABLE1)

        # Filter Table 2
        match = parser.OFPMatch(in_port=3)
        inst = []
        self.add_flow(datapath=datapath, priority=10,  match=match, inst=inst, table_id=FILTER_TABLE2)
        ...
        match = parser.OFPMatch()
        inst = [parser.OFPInstructionGotoTable(FORWARD_TABLE)]
        self.add_flow(datapath=datapath, priority=0,  match=match, inst=inst, table_id=FILTER_TABLE2)
        ...
        self.add_flow(datapath=datapath, priority=0,  match=match, inst=inst, table_id=FORWARD_TABLE)



    def add_flow(self, datapath, priority, match, inst, table_id=0):
